...
DATA_PATH=$PWD/data/arabic-egy-msa
...
# files full paths
SRC_RAW=$MONO_PATH/all.egy
TGT_RAW=$MONO_PATH/all.msa
SRC_TOK=$MONO_PATH/all.egy.tok
TGT_TOK=$MONO_PATH/all.msa.tok
BPE_CODES=$MONO_PATH/bpe_codes
CONCAT_BPE=$MONO_PATH/all.egy-msa.$CODES
SRC_VOCAB=$MONO_PATH/vocab.egy.$CODES
TGT_VOCAB=$MONO_PATH/vocab.msa.$CODES
FULL_VOCAB=$MONO_PATH/vocab.egy-msa.$CODES
SRC_VALID=$PARA_PATH/dev/parallelsentences-6.egy-valid
TGT_VALID=$PARA_PATH/dev/parallelsentences-6.msa-valid
SRC_TEST=$PARA_PATH/dev/parallelsentences-7.egy-train
TGT_TEST=$PARA_PATH/dev/parallelsentences-7.msa-train
SRC_TRAIN=$PARA_PATH/dev/parallelsentences-6.egy-train
TGT_TRAIN=$PARA_PATH/dev/parallelsentences-6.msa-train
...
if true && ! [[ -f "$SRC_RAW" && -f "$TGT_RAW" ]]; then
  echo "Concatenating monolingual data..."
  echo "starttttttttttttttttttttttt $(ls)"
  echo "endddddddddddddddddddddddddd "
  
  cat $(ls DIAL*EGY* | grep -v gz) | head -n $N_MONO > $SRC_RAW
  cat $(ls msa*ar* | grep -v gz) | head -n $N_MONO > $TGT_RAW
fi
echo "EN monolingual data concatenated in: $SRC_RAW"
echo "FR monolingual data concatenated in: $TGT_RAW"

# check number of lines
if true && ! [[ "$(wc -l < $SRC_RAW)" -eq "$N_MONO" ]]; then echo "ERROR: Number of lines doesn't match! Be sure you have $N_MONO sentences in your EN monolingual data."; exit; fi
if true && ! [[ "$(wc -l < $TGT_RAW)" -eq "$N_MONO" ]]; then echo "ERROR: Number of lines doesn't match! Be sure you have $N_MONO sentences in your FR monolingual data."; exit; fi
...
if ! [[ -f "$CONCAT_BPE.vec" ]]; then
  ...
  $FASTTEXT skipgram -epoch $N_EPOCHS -minCount 0 -dim 300 -thread $N_THREADS -ws 5 -neg 10   -input $CONCAT_BPE -output $CONCAT_BPE  -lr 0.06 
fi
